...
const setSeriesSequenceCache =  (numberBox) => {
    return redis.set(seriesSequenceKey, JSON.stringify(numberBox), (err) => {
        return err
    });
}

function getSeriesSequenceCache() {
    let numberBox = { 1: 3, 2: 5, 3: 9, 4: 15 };
...
async function findSeriesSequence(n) {
    ...
    if (seriesSequence[n]) {
        ...
        let next = await findSeriesSequence(n - 1) + 2 * (n - 1);
...
router.get('/:n',async (req, res) => {
    ...
    if (seriesSequence[req.params.n]) {
        ...
        let data = await findSeriesSequence(req.params.n);
        if(data) {
